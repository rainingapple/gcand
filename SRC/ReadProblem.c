...
//static void Read_NODE_COORD_SECTION(char* tspstr, int pos);
    ...
    while (Line) {
        ...
        if (!strcmp(Keyword, "COMMENT"));
            ...
            for (i = 1; i <= Dimension; i++) {
                N = &NodeSet[i];
                tmp = ReadLine(tspstr, &pos);
                char noth[20], xcord[20], ycord[100];
                sscanf(tmp, "%s %s %s", noth, xcord, ycord);
                N->X = atof(xcord);
                N->Y = atof(ycord);
            }
        }
...
//static void Read_NODE_COORD_SECTION(char* tspstr)
//{
//    Node* N;
//    int Id, i;
//    char* tmp;
